...
## How long it has been cheap

The `screen` block may also carry a `trend`, with the `history` it was derived from: the
same screens run 12 and 24 months earlier. A company that has been cheap for two years
and one that fell into the screen this year are different animals, and the questions
worth asking differ:

- **persistent** — flagged at every date. The market has had two years to change its
  mind and has not. Look for what the filing keeps saying, and for a risk that has been
  restated in the same words for three years while the business moved underneath it.
- **new** — flagged now and at neither earlier date. Something changed. Look for what,
  and read `history` before assuming it was the price: a company that was *excluded*
  earlier is new to the data, not newly cheap, and that is not a finding about the
  business at all.
- **returning** — flagged two years ago, not last year, flagged again now. Look for
  whether the reason is the same one as before or a different one.
- **recent** — flagged last year and now, but not two years ago.

`history` records each earlier date with what flagged the company then, or why it was
excluded. Trust it over the label: the label is arithmetic over the history, and the
history is what actually happened. None of this is evidence about the filing — it cannot
be quoted, so it can never be a finding on its own. It decides what you read for.
